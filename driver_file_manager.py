...
class DriverFileManager:
    def __init__(self, base_dir="driver_records"):
    ...
    def organize_delivery_data(self, current_driver=None, source_file='delivery_data.json'):
        if current_driver is None:
            return
        month_dir = os.path.join(self.base_dir, current_driver, 
                                f"{datetime.now().year}_{datetime.now().month:02d}")
        os.makedirs(month_dir, exist_ok=True)
        excel_path = os.path.join(month_dir, 'deliveries.xlsx')
        ...
        with open(source_file, 'r', encoding='utf-8') as f:
            for line in f:
                try:
                    ...
                    self.update_delivery_excel(excel_path, data)
                except json.JSONDecodeError:
                    continue
